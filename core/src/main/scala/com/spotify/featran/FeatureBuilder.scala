...
object FeatureBuilder {
  ...
  implicit def mapFB[T: ClassTag : FloatingPoint]: FeatureBuilder[Map[String, T]] =
    new FeatureBuilder[Map[String, T]] { self =>
      private var m: _root_.java.util.Map[String, T] = _
      private val fp = implicitly[FloatingPoint[T]]
      override def init(dimension: Int): Unit = m = new _root_.java.util.HashMap[String, T]
...
private class JMapWrapper[K, V](val m: _root_.java.util.Map[K, V]) extends Map[K, V] {
  ...
  override def iterator: Iterator[(K, V)] =
    m.entrySet().iterator().asScala.map(kv => (kv.getKey, kv.getValue))
}
